...
def fetch_price(symbol,producer,topic_name):
	...
	try:
		response = requests.get('%s/products/%sticker' % (API_BASE,symbol))
		...
		logger.debug('Sent price for %s to kafks', symbol)
	except KafkaTimeoutError as timeout_error:
		logger.warn('Failed to send price to price to kafka, caused by: %s', (timeout_error.message));
